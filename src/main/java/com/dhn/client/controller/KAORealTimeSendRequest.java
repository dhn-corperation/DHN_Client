...
@Component
@Slf4j
public class KAORealTimeSendRequest implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        if(dbug.equalsIgnoreCase("Y")){
            log.info("Real Time KAO setting value : " + param.toString());
        }
    }
    ...
    private void SendProcess() {
        if(isStart && !isProc) {
            ...
            if(activeThreads < 2){
                ...
                String group_no = "RK" + now.format(formatter);

                if(!group_no.equals(preGroupNo)){
                    try{
                        int cnt = requestService.selectRealKAORequestCount(param);

                        if(cnt > 0){
                            param.setGroup_no(group_no);
                            requestService.kaoRealGroupUpdate(param);

                            executorService.submit(() -> APIProcess(group_no));

                        }
                    }catch (Exception e){
                        log.error("Real Time KAO 메세지 전송 오류(Send) : " + e.toString());
                    }
                    preGroupNo = group_no;
                }
            }
    ...
    private void APIProcess(String group_no) {
        try{
            ...
            List<KAORequestBean> _list = requestService.selectRealKAORequests(sendParam);
            ...
            if(phnerr_msgid.size() > 0){
                ...
                log.info("Real Time KAO {} 건 번호체크 오류", phnerr_msgid.size());
            }
            ...
            if(syserr_msgid.size() > 0){
                ...
                requestService.phnErrUpdateDelete(_ml);
                log.info("Real Time KAO {} 건 미등록 시스템코드", syserr_msgid.size());
            }
            ...
            if(dateerr_msgid.size() > 0){
                ...
                requestService.phnErrUpdateDelete(_ml);
                log.info("Real Time KAO {} 건 지난 발송일자", dateerr_msgid.size());
            }
            ...
            if(sendList.size() > 0){
                ...
                if(dbug.equalsIgnoreCase("Y")){
                    log.info("Real Time KAO data : " + sw.toString());
                }
                ...
                try {
                    ...
                    if (response.getStatusCode() == HttpStatus.OK) {
                        ...
                        log.info("Real Time KAO 메세지 전송 완료(" + response.getStatusCode() + ") : "+ sendList.size() + " 건");
                    } else {
                        log.error("Real Time KAO 메세지 전송 오류(Http ERR) : " + res.get("userid") + " / " + res.get("message"));
                        requestService.updateKAOSendInit(sendParam);
                    }
                } catch (Exception e) {
                    log.error("Real Time KAO 메세지 전송 오류(Response) : " + e.toString());
                    requestService.updateKAOSendInit(sendParam);
                }
            }
        } catch (Exception e){
    ...
    static public void setIsStart(boolean _flag) {
        log.info(role + " Real Time KAO Process is change : " + _flag);
        isStart = _flag;
    }
