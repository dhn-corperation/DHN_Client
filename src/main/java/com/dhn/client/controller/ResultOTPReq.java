...
@Component
@Slf4j
public class ResultOTPReq implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private void SendProcess() {
        ...
        if(isStart && !isProc && activeThreads < 10) {
            ...
            try {
                ...
                try {
                    ResponseEntity<String> response = rt.postForEntity(dhnServer + "resultotp", entity, String.class);
    ...
    private void ResultProc(JSONArray json) {
        ...
        for(int i=0; i<json.length(); i++) {
            ...
            try {
                ...
                log.info("OTP 결과 처리 오류 [ " + _ml.getMsgid() + " ] - " + e.toString());
            }
        }
        log.info("OTP 결과 처리 완료 [ {} ] 건",json.length());

    }

    static public void setIsStart(boolean _flag) {
        log.info(role + " OTP Result Request is  change : " + _flag);
        isStart = _flag;
    }
}
