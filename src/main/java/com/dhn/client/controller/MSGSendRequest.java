            ...
            if(activeThreads < 3){
                ...
                if(!group_no.equals(preGroupNo)){
                    try{
                        ...
                        log.error("MSG L 메세지 전송 오류(Send) : " + e.toString());
                    }
                    preGroupNo = group_no;
                }
            }
...
//            log.info(sw.toString());
            ...
            try {
                ...
                if (response.getStatusCode() == HttpStatus.OK) {
                    ...
                    log.info("MSG L 메세지 전송 완료(" + response.getStatusCode() + ") : "+ _list.size() + " 건");
                } else {
                    log.error("MSG L 메세지 전송 오류(Http ERR) : " + res.get("userid") + " / " + res.get("message"));
                    requestService.updateMessageInit(sendParam);
                }
            } catch (Exception e) {
                log.error("MSG L 메세지 전송 오류(Response) : " + e.toString());
                requestService.updateMessageInit(sendParam);
            }
