...
@Component
@Slf4j
public class SMSRealTimeSendRequest implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        if(dbug.equalsIgnoreCase("Y")){
            log.info("Real Time SMS setting value : " + param.toString());
        }
    }
    ...
    private void SendProcess() {
        if(isStart && !isProc) {
            ...
            if(activeThreads < 1){
                ...
                String group_no = "RS"+now.format(formatter);
                ...
                if(!group_no.equals(preGroupNo)){
                    try{
                        ...
                        log.error("Real Time SMS 메세지 전송 오류(Send) : " + e.toString());
                    }
                    preGroupNo = group_no;
                }
            }
    ...
    private void APIProcess(String group_no) {
        ...
        try{
            ...
            if(phnerr_msgid.size() > 0){
                ...
                log.info("Real Time SMS {} 건 번호체크 오류", phnerr_msgid.size());
            }
            ...
            if(syserr_msgid.size() > 0){
                ...
                requestService.phnErrUpdateDelete(_ml);
                log.info("Real Time SMS {} 건 미등록 시스템코드", syserr_msgid.size());
            }
            ...
            if(dateerr_msgid.size() > 0){
                ...
                requestService.phnErrUpdateDelete(_ml);
                log.info("Real Time SMS {} 건 지난 발송일자", dateerr_msgid.size());
            }
            ...
            if(sendList.size() > 0){
                ...
                if(dbug.equalsIgnoreCase("Y")){
                    log.info("Real Time LMS data : " + sw.toString());
                }
                ...
                try {
                    ...
                    if (response.getStatusCode() == HttpStatus.OK) {
                        ...
                        log.info("Real Time SMS 메세지 전송 완료(" + response.getStatusCode() + ") : "+ _list.size() + " 건");
                    } else {
                        log.error("Real Time SMS 메세지 전송 오류(Http ERR) : " + res.get("userid") + " / " + res.get("message"));
                        requestService.updateMSGSendInit(sendParam);
                    }
                } catch (Exception e) {
                    log.error("Real Time SMS 메세지 전송 오류(Response) : " + e.toString());
                    requestService.updateMSGSendInit(sendParam);
                }
            }
        }catch (Exception e){
    ...
    static public void setIsStart(boolean _flag) {
        log.info(role + " Real Time SMS Process is  change : " + _flag);
        isStart = _flag;
    }
}
