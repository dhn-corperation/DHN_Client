...
@Component
@Slf4j
public class SMSTranSendRequest implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private void SendProcess() {
        if (isStart && !isProc) {
            ...
            if (!group_no.equals(preGroupNo)) {
                try {
                    int cnt = requestService.selectSMSTranRequestCount(param);

                    if (cnt > 0) {
                        param.setGroup_no(group_no);

                        requestService.updateSMSTranGroupNo(param);

                        List<RequestBean> _list = requestService.selectSMSTranRequests(param);
                        ...
                        try {
                            ...
                            if (response.getStatusCode() == HttpStatus.OK) {
                                requestService.updateSMSTranSendComplete(param);
...
//                                Map<String, String> res = om.readValue(response.getBody().toString(), Map.class);
                                ...
                                requestService.updateSMSTranSendInit(param);
                            }
                        } catch (Exception e) {
