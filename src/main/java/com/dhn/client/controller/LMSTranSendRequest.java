...
@Component
@Slf4j
public class LMSTranSendRequest implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private void SendProcess() {
        if (isStart && !isProc) {
            ...
            if (!group_no.equals(preGroupNo)) {
                ...
                try {
                    int cnt = requestService.selectLMSTranRequestCount(param);

                    if (cnt > 0) {

                        param.setGroup_no(group_no);

                        requestService.updateLMSTranGroupNo(param);

                        List<RequestBean> _list = requestService.selectLMSTranRequests(param);
