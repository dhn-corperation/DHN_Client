...
public class ImmediateKAORequest implements ApplicationListener<ContextRefreshedEvent> {
    ...
    private void SendProcess() {
        if(isStart && !isProc) {
            ...
            if(activeThreads < 5){
                ...
                String group_no = "IK" + now.format(formatter);
                ...
                if(!group_no.equals(preGroupNo)){
                    try{
                        ...
                        log.error("Immediate KAO 메세지 전송 오류(Send) : " + e.toString());
                    }
                    preGroupNo = group_no;
                }
            }
    ...
    private void APIProcess(String group_no) {
        try{
            ...
            List<MessageRequestBean> _list = requestService.selectKaoImmediateRequests(sendParam);
...
//            log.info(sw.toString());
            ...
            try {
                ...
                if (response.getStatusCode() == HttpStatus.OK) {
                    ...
                    log.info("Immediate KAO 메세지 전송 완료(" + response.getStatusCode() + ") : "+ _list.size() + " 건");
                } else {
                    log.error("Immediate KAO 메세지 전송 오류(Http ERR) : " + res.get("userid") + " / " + res.get("message"));
                    requestService.updateMessageInit(sendParam);
                }
            } catch (Exception e) {
                log.error("Immediate KAO 메세지 전송 오류(Response) : " + e.toString());
                requestService.updateMessageInit(sendParam);
            }
