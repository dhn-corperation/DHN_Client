...
@Component
public class KAOtoSMS implements ApplicationListener<ContextRefreshedEvent>{
	...
	private void SendProcess() {
		if(isStart && !isProc) {
			...
			try {
				int cnt = reqService.kaox_to_sms_count(param);

				if(cnt > 0) {
					try {
						reqService.kaox_to_tran_type_update(param);
					} catch (Exception e) {
				...
				log.error("알림톡 Table > Tran_type Update 오류 : " + e.toString());
			}
			...
			if(!group_no.equals(preGroupNo))
			{
				...
				try {
					int cnt = reqService.kakao_to_sms_count(param);

					if(cnt > 0) {
						param.setGroup_no(group_no);
						try {
							reqService.kakao_to_sms_group_update(param);
						} catch (Exception e) {
							// TODO: handle exception
						}finally {
							reqService.kakao_to_sms_move(param);
						}
					}					   
					...
					//e.printStackTrace();
					log.error("알림톡 Table > SMS Table 오류 : " + e.toString());
				}
				preGroupNo = group_no;
			}
